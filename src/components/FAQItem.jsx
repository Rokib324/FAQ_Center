...
const FAQItem = ({ item, onClick, openId }) => {
  ...
  return (
    <div className='border-b border-gray-200 dark:border-gray-700 last:border-none transition-colors duration-300 hover:bg-gradient-to-r hover:from-gray-50/50 hover:to-transparent dark:hover:from-gray-800/50 dark:hover:to-transparent' id={`faq-item-${item.id}`}>
      ...
      <button className={`w-full py-5 px-4 flex items-center justify-between text-left focus:outline-none rounded-lg transition-all duration-300 cursor-pointer text-gray-900 dark:text-white hover:text-transparent hover:bg-clip-text hover:bg-gradient-to-r hover:from-indigo-600 hover:to-purple-600 dark:hover:from-indigo-400 dark:hover:to-purple-400 ${openId ? 'bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/50 dark:to-purple-900/50' : ''}`} onClick={() => onClick(item.id)}>
        ...
        <div className='flex-shrink-0 flex items-center justify-center w-8 min-w-8 aspect-square rounded-full bg-gray-200 dark:bg-gray-700 transition-all duration-300'>
          <i className={`bx bx-${openId ? 'minus' : 'plus'} text-gray-500 hover:text-gray-600 dark:text-gray-400 dark:hover:text-white text-xl`}></i>
        </div>
      </button>
      <div className='overflow-hidden transition-all duration-200 ease-out' id={`answer-${item.id}`} ref={answerRef} style={{maxHeight: openId ? answerRef.current?.scrollHeight : 0, opacity:openId ? 1 : 0}}>
